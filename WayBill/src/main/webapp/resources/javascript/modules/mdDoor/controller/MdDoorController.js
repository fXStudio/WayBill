Ext.define('MdDoorModule.controller.MdDoorController', {
    extend: 'Ext.app.Controller',
    refs: [
       {ref: 'gridPanel', selector: 'mddoorgrid'},
       {ref: 'window', selector: 'mddoorwindow'},
       {ref: 'formPanel', selector: 'mddoorform'},
       {ref: 'submitBtn', selector: 'mddoorwindow button[action=submit]'}
    ],
    ...
    init: function() {
	    this.control({'textfield[name=searchField]': {
	                    ...
	                    gridPanel.getStore().filter({
	                        filterFn: function(item) {
	                            return !field.getValue() || 
	                                   item.get("name") && item.get("name").indexOf(field.getValue()) > -1; 
	                        }
	                    });
	                }
	            }
	    	...
	    	'button[action=add]':{
	    		click: function(){
	    			// 获得窗体对象的引用
	            	var mdDoorWindow = this.getWindow();
	            	// 判断窗体对象是否存在, 如果不存在，就创建一个新的窗体对象
	            	if(!mdDoorWindow){mdDoorWindow = Ext.create('MdDoorModule.view.MdDoorWindow');}
	            	// 在执行新增业务的时候，要把窗体的内容清空
	                this.getFormPanel().getForm().reset();
	                
	                mdDoorWindow.show(); // 显示窗体
	                mdDoorWindow.center();// 窗体居中显示
	            }
	        },
	        	...
	        	click: function(){
	                ...
	                if (formObj.isValid()) {
	                    formObj.submit({
	                        ...
	                        waitTitle: '提示', // 标题  
	                        url: 'services/mdDoorModify', // 请求的url地址  
	                        method: 'POST', // 请求方式  
