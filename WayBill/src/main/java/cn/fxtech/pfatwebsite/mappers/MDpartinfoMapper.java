...
public interface MDpartinfoMapper extends Mapper<MDpartinfo> {
	@Select("select * from terminal_partnifo where destination_id = #{destinationId} and not exists("
			+ " select id from terminal_orderpart where cqadno= partno and orderid = #{orderid})")
	...
	@Select("select a.id, cqadno, cdesc, cquantity, b.destination, isscan from terminal_partnifo a inner join terminal_destination b on a.destination_id = b.id")
