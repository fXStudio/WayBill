Ext.define('MdPartInfoModule.controller.MdPartInfoController', {
    extend: 'Ext.app.Controller',
    refs: [
       {ref: 'gridPanel', selector: 'mdpartinfogrid'},
       {ref: 'window', selector: 'mdpartinfowindow'},
       {ref: 'formPanel', selector: 'mdpartinfoform'},
       {ref: 'submitBtn', selector: 'mdpartinfowindow button[action=submit]'}
    ],
	    ...
	    this.control({'textfield[name=searchField]': {
	    	...
	    	'button[action=add]':{
	    		click: function(){
	            	...
	            	if(!win){win = Ext.create('MdPartInfoModule.view.MdPartInfoWindow');}
	            	// 在执行新增业务的时候，要把窗体的内容清空
	                this.getFormPanel().getForm().reset();
	                
	                Ext.getCmp('destination').getStore().load();
	        	...
	        	click: function(){
	                ...
	                if (formObj.isValid()) {
	                    formObj.submit({
	                        ...
	                        waitTitle: '提示', // 标题  
	                        url: 'services/partinfoModify', // 请求的url地址  
	                        method: 'POST', // 请求方式  
