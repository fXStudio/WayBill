Ext.define('MdCarInfoModule.controller.MdCarInfoController', {
    extend: 'Ext.app.Controller',
    refs: [
       {ref: 'gridPanel', selector: 'mdcarinfogrid'},
       {ref: 'window', selector: 'mdcarinfowindow'},
       {ref: 'formPanel', selector: 'mdcarinfoform'},
       {ref: 'submitBtn', selector: 'mdcarinfowindow button[action=submit]'}
    ],
    ...
    init: function() {
	    this.control({'textfield[name=searchField]': {
	    	...
	    	'button[action=add]':{
	    		click: function(){
	    			// 获得窗体对象的引用
	            	var mdCarInfoWindow = this.getWindow();
	            	// 判断窗体对象是否存在, 如果不存在，就创建一个新的窗体对象
	            	if(!mdCarInfoWindow){mdCarInfoWindow = Ext.create('MdCarInfoModule.view.MdCarInfoWindow');}
	            	// 在执行新增业务的时候，要把窗体的内容清空
	                this.getFormPanel().getForm().reset();
	                
	                mdCarInfoWindow.show(); // 显示窗体
	                mdCarInfoWindow.center();// 窗体居中显示
	            }
	        },
	        	...
	        	click: function(){
	                ...
	                if (formObj.isValid()) {
	                    formObj.submit({
	                        ...
	                        waitTitle: '提示', // 标题  
	                        url: 'services/mdCarInfoModify', // 请求的url地址  
	                        method: 'POST', // 请求方式  
